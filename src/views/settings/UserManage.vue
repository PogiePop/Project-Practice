    ...
    <div class="usermg-page">
        <el-card class="table-card" shadow="never">
            ...
            <el-table :data="tableData" stripe style="width:100%" height="calc(100vh - 200px)">
                ...
                <el-table-column prop="roleLevel" label="角色" width="100">
                    <template #default="{ row }">
                        <el-tag :type="row.roleLevel === 0 ? 'danger' : 'primary'" size="small">{{ row.roleLevel === 0 ? '超级管理员' : '审计员' }}</el-tag>
                    </template>
                </el-table-column>
        ...
        <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑用户' : '新建用户'" width="500px" draggable :close-on-click-modal="false">
            <el-form ref="formRef" :model="userForm" :rules="rules" label-width="80px">
                ...
                <el-form-item v-if="!isEdit" label="密码"><el-input v-model="userForm.password" type="password" placeholder="默认123456" /></el-form-item>
                <el-form-item label="姓名" prop="realName"><el-input v-model="userForm.realName" /></el-form-item>
                <el-row :gutter="16"><el-col :span="12"><el-form-item label="工号"><el-input v-model="userForm.staffId" /></el-form-item></el-col><el-col :span="12"><el-form-item label="角色"><el-select v-model="userForm.roleLevel"><el-option :value="1" label="审计员" /><el-option :value="0" label="超级管理员" /></el-select></el-form-item></el-col></el-row>
