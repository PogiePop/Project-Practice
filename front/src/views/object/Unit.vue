...
const filteredUnitLeaders = computed(() => {
    if (!unitForm.unitName) return leaderSelectList.value
    return leaderSelectList.value.filter(l => l.currentUnitName === unitForm.unitName && l.isActive === 1)
})
const onLeaderInChargeChange = (val) => {
    const leader = leaderSelectList.value.find(l => l.leaderName === val)
    if (leader && leader.phone) unitForm.leaderInChargePhone = leader.phone
    else unitForm.leaderInChargePhone = ''
}
const onFinanceContactChange = (val) => {
    const leader = leaderSelectList.value.find(l => l.leaderName === val)
    if (leader && leader.phone) unitForm.financeContactPhone = leader.phone
    else unitForm.financeContactPhone = ''
}
...
const openDetail = async (row) => { detailUnit.value = { ...row, rectifyLedger: [] }; detailDrawerVisible.value = true; try { const rl = await objectApi.getUnitRectifyLedger(row.unitId); detailUnit.value.rectifyLedger = rl?.list || [] } catch {} }
...
const unitExportColumns = [
    { prop: 'unitId', label: '单位编码' },{ prop: 'unitName', label: '单位名称' },
    { prop: 'categoryName', label: '分类' },{ prop: 'establishmentCount', label: '编制人数' },
    { prop: 'leaderInCharge', label: '分管校领导' },
    { prop: 'leaderInChargePhone', label: '分管领导电话' },
...
const unitImportTemplateFields = [
    { label: '单位编码', example: 'UNIT001', required: false },
    { label: '单位名称', example: 'XX学院', required: true },
    { label: '分类', example: '1-二级学院', required: true },
    { label: '编制人数', example: '85', required: false },
    { label: '分管校领导', example: '张XX', required: false },
    { label: '分管领导电话', example: '13800000001', required: false },
    { label: '财务联系人', example: '李XX', required: false },
    { label: '联系电话', example: '13800000002', required: false },
...
const confirmImport = async () => { if (!importPreviewData.value.length) { ElMessage.warning('无数据'); return }; try { const categoryMap = { '校内职能部门': 0, '二级学院': 1, '直属后勤单位': 2, '校办企业': 3, '基建项目部': 4, '附属医院': 5 }; const existingIds = new Set(tableData.value.map(r => r.unitId)); let added = 0, updated = 0; for (const row of importPreviewData.value) { const payload = { unitName: row['单位名称'] || row['名称'] || '未命名', category: parseInt(row['分类']) || categoryMap[row['分类']] || 0, establishmentCount: parseInt(row['编制人数']) || 0, leaderInCharge: row['分管校领导'] || '', leaderInChargePhone: row['分管领导电话'] || '', financeContact: row['财务联系人'] || '', financeContactPhone: row['联系电话'] || '', address: row['地址'] || '', setupDate: row['设立日期'] || undefined, totalAuditCount: parseInt(row['审计次数']) || 0, latestAuditDate: row['最近审计日期'] || null, pendingRectifyCount: parseInt(row['待整改数']) || 0 }; const uid = row['单位编码'] || row['编码'] || ''; if (uid && existingIds.has(uid)) { await objectApi.updateUnit(uid, payload); updated++ } else { await objectApi.createUnit({ ...payload, unitId: uid || undefined }); added++ } }; if (added || updated) commonApi.sendImportMsg('被审计单位', added, updated).then(() => window.dispatchEvent(new Event('msg-refresh')))
            importVisible.value = false; fetchUnitTable()
    ...
    <div class="unit-page">
        ...
        <el-dialog v-model="dialogVisible" :title="dialogTitle" width="580px" draggable :close-on-click-modal="false">
            <el-form ref="formRef" :model="unitForm" :rules="rules" label-width="110px">
                ...
                <el-form-item label="编制人数"><el-input-number v-model="unitForm.establishmentCount" :min="0" style="width:100%" /></el-form-item>
                <el-form-item label="分管校领导"><el-select v-model="unitForm.leaderInCharge" placeholder="请选择分管校领导" clearable filterable style="width:100%"><el-option v-for="l in filteredUnitLeaders" :key="l.leaderId" :label="l.leaderName" :value="l.leaderName" /></el-select></el-form-item>
                <el-form-item label="财务联系人"><el-select v-model="unitForm.financeContact" @change="onFinanceContactChange" placeholder="请选择财务联系人" clearable filterable style="width:100%"><el-option v-for="l in filteredUnitLeaders" :key="l.leaderId" :label="l.leaderName" :value="l.leaderName" /></el-select></el-form-item>
        ...
        <el-drawer v-model="detailDrawerVisible" title="单位详情" size="580px">
            <template v-if="detailUnit">
                <el-descriptions :column="2" border size="small"><el-descriptions-item label="编码">{{ detailUnit.unitCode }}</el-descriptions-item><el-descriptions-item label="名称">{{ detailUnit.unitName }}</el-descriptions-item><el-descriptions-item label="分类">{{ detailUnit.categoryName }}</el-descriptions-item><el-descriptions-item label="编制">{{ detailUnit.establishmentCount }}</el-descriptions-item><el-descriptions-item label="分管领导">{{ detailUnit.leaderInCharge }}</el-descriptions-item><el-descriptions-item label="分管领导电话">{{ detailUnit.leaderInChargePhone || '-' }}</el-descriptions-item><el-descriptions-item label="财务联系人">{{ detailUnit.financeContact }}</el-descriptions-item><el-descriptions-item label="联系电话">{{ detailUnit.financeContactPhone }}</el-descriptions-item><el-descriptions-item label="地址">{{ detailUnit.address || '-' }}</el-descriptions-item><el-descriptions-item label="设立日期">{{ detailUnit.setupDate || '-' }}</el-descriptions-item></el-descriptions>
