...
import { Lock, User, View, Hide, RefreshRight } from '@element-plus/icons-vue'
import { commonApi } from '@/api'
import { resetExpiredLock } from '@/api/request'
...
const refreshCaptcha = async () => {
  ...
  try {
    captchaUuid.value = genUuid()
    const data = await commonApi.getCaptcha(captchaUuid.value)
...
// 页面加载时获取验证码
// 注意: 不在onMounted重置过期锁 —— 主动退出时锁需保持以阻止旧页面pending请求的401弹窗
// 过期锁仅在 handleLogin 登录成功后通过 resetExpiredLock() 释放
onMounted(() => {
  refreshCaptcha()
})
        ...
        <el-form-item prop="captchaCode">
          <div class="captcha-row">
            <el-input
              ...
              clearable
            />
            <div class="captcha-img-wrapper" @click="refreshCaptcha" :class="{ loading: captchaLoading }">
              <img v-if="captchaImage" :src="captchaImage" alt="验证码" class="captcha-img" />
              <el-icon v-else :size="20"><RefreshRight /></el-icon>
...
.captcha-img-wrapper {
  width: 110px; height: 42px; flex-shrink: 0; border-radius: 8px; overflow: hidden;
  cursor: pointer; position: relative; border: 1px solid #e4e7ed;
  display: flex; align-items: center; justify-content: center; background: #f5f7fa;
}
.captcha-img-wrapper.loading { pointer-events: none; opacity: 0.6; }
...
.captcha-img-wrapper:hover .captcha-mask { opacity: 1; }
